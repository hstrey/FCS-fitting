...
from fcsfit.realistic_rev_oligo import g_oligo_all_nc
...
if (bluePick==1 or bluePick==0) and (redPick==1 or redPick==0):
    ...
    if (bluePick==3 or bluePick==5):
        ...
        out=minimize(g_oligo_all_nc,b,args=(t,data,dataStd_fit))
...
dataFits.to_csv(oligodir+'oligo_c_fits_'+str(bluePick)+'_'+str(redPick)+'.csv')
...
plt.savefig(oligodir+'oligo_c_'+str(bluePick)+'_'+str(redPick)+'.png', bbox_inches='tight')
plt.savefig(oligodir+'oligo_c_'+str(bluePick)+'_'+str(redPick)+'.pdf', bbox_inches='tight')
plt.show()
