...
# empirically p=0.33
# does not work for out data though
...
#####################################################################
#constants
maxz=40 # z integration range in microns
...
def g_all_nr(b,t,c,data=None,sigma=None):
    ...
    for conc in c:
        ...
        g=[1+g_hermite(tt,D,w0,a,r0,lambdaex,lambdaem,n,k=k_real)[0]/C/v1/v1 for tt in t]
        if corr_g is None:
            corr_g=g
        else:
            corr_g=np.vstack((corr_g,g))
...
def g_all_ntr(b,t,c,data=None,sigma=None):
    ...
    for conc in c:
        ...
        g=[1+(1-F+F*np.exp(-tt/tf))/(1-F)*g_hermite(tt,D,w0,a,r0,lambdaex,lambdaem,n,k=k_real)[0]/C/v1/v1 for tt in t]
        if corr_g is None:
            corr_g=g
        else:
            corr_g=np.vstack((corr_g,g))
...
def gc_hermite(t,D,w1,w2,a1,a2,R1,R2,lambdaex1,lambdaem1,lambdaex2,lambdaem2,n,dz):
    return quad(gcz1_hermite,-maxz,maxz,args=(t,D,w1,w2,a1,a2,R1,R2,lambdaex1,lambdaem1,lambdaex2,lambdaem2,n,dz))
...
def g0f(z,w11,w22,a1,a2,R1,R2,lambdaex1,lambdaem1,lambdaex2,lambdaem2,n,dz):
...
def vol2c(w1,w2,a1,a2,R1,R2,lambdaex1,lambdaem1,lambdaex2,lambdaem2,n,dz):
    return np.pi/4.0*quad(g0c,-maxz,maxz,args=(w1,w2,a1,a2,R1,R2,lambdaex1,lambdaem1,lambdaex2,lambdaem2,n,dz))[0]

def vol2f(w1,w2,a1,a2,R1,R2,lambdaex1,lambdaem1,lambdaex2,lambdaem2,n,dz):
    return np.pi/4.0*quad(g0f,-maxz,maxz,args=(w1,w2,a1,a2,R1,R2,lambdaex1,lambdaem1,lambdaex2,lambdaem2,n,dz))[0]

def g_nc(t,D,C,w1,w2,a1,a2,R1,R2,lambdaex1,lambdaem1,lambdaex2,lambdaem2,n,dz):
    ...
    return np.array([1.0+gc_hermite(tt, D, w1,w2,a1,a2,R1,R2,lambdaex1,lambdaem1,lambdaex2,lambdaem2,n,dz)[0]/C/6.022e-1/v1/v2/2.0 for tt in t])
...
#    gDr =np.array(gDr)
    ...
    g=[1.0+gc_hermite(tt, D, wxy_b, wxy_r, a_b, a_r, r0_b, r0_r, lambdaex_b,lambdaem_b,lambdaex_r,lambdaem_r,n,dz)[0]/C/v1/v2/2.0 for tt in t]
#    gDc=np.array(gDc)
    corr_g=np.vstack((corr_g,g))
...
#    gDr =np.array(gDr)
    ...
    v1=vol1(a_b,r0_b,lambdaem_b,n)
    v2=vol1(a_r,r0_r,lambdaem_r,n)
    g=[1.0+gc_hermite(tt, D, wxy_b, wxy_r, a_b, a_r, r0_b, r0_r, lambdaex_b,lambdaem_b,lambdaex_r,lambdaem_r,n,dz)[0]/Cc/v1/v2/2.0 for tt in t]
#    gDc=np.array(gDc)
    corr_g=np.vstack((corr_g,g))
