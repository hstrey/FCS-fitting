...
logfile=open(datadir_all+'oligo_'+fit_type+str(bluePick)+'_'+str(redPick)+'.log',"w")
...
if (bluePick==1 or bluePick==0) and (redPick==1 or redPick==0):
    ...
    b.add('wz_r',value=bRed['wz'].value,vary=True)
    b.add('wxy_b',value=bBlue['wxy'].value,vary=False)
    b.add('wxy_r',value=bRed['wxy'].value,vary=True)
    ...
    b.add('w0_r',value=bRed['w0'].value,vary=True)
...
dataFits.to_csv(oligodir+'oligo_fits_'+fit_type+str(bluePick)+'_'+str(redPick)+'.csv')
...
plt.savefig(oligodir+'oligo_'+fit_type+str(bluePick)+'_'+str(redPick)+'.png', bbox_inches='tight')
plt.savefig(oligodir+'oligo_'+fit_type+str(bluePick)+'_'+str(redPick)+'.pdf', bbox_inches='tight')
plt.show()
