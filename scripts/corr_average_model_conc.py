...
logfile=open(datadir+parafile+'_conc.log',"w")
...
for color in ['B','R']:
    ...
    #            tf=1e-7,
        ...
        plt.ylabel('sigma')
        plt.xlabel('delta t in sec')
        plt.savefig(datadir+filename+color+parafile+'_conc.png', bbox_inches='tight')
        plt.savefig(datadir+filename+color+parafile+'_conc.pdf', bbox_inches='tight')
logfile.close()

data3dGaussian.to_csv(datadir+'gaussian_conc_'+parafile+'.csv')
data3dGaussianTriplet.to_csv(datadir+'gaussian_triplet_conc_'+parafile+'.csv')
dataNumerical.to_csv(datadir+'Numerical_conc_'+parafile+'.csv')
dataNumericalTriplet.to_csv(datadir+'NumericalTriplet_conc_'+parafile+'.csv')
