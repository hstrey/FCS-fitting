...
for i in range(len(experiments)):
    ...
    for color in ['B','R']:
        ...
        #create a new row of data and append to result
        datasetT=makeResultDataFrame(resultS,{'dataset':filename,'color':color,'v':resultG.values['wxy']**2*resultG.values['wz']*m.pi**1.5})
        data3dGaussianTriplet=data3dGaussianTriplet.append(datasetG,ignore_index=True)
...
#            tf=1e-7,
            ...
            F=Parameter(value=resultS.values['F'],min=0.0,max=0.5,vary=True),
            tf=Parameter(value=resultS.values['tf'],vary=True),
            n=Parameter(value=1.33, vary=False),
            D=Parameter(value=diffC, vary = False),
            a=Parameter(value=a,vary=False),
            lambdaem=Parameter(value=lambdaem,vary=False),
